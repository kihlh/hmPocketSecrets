...
var hmc = require("hmc-win32");
var nsfw = require("pre-nsfw");
var path = require("path");
var shake = require("hmc-shake");
var log = require("electron-log");
log.initialize({ preload: true });
var nameList = ["DeviceTube.rs", "libNcPlus.rs", "main_view.rs", "main.rs", "util.rs"];
log.info("::nameList:: " + nameList.join(","));
nsfw(path.resolve("./"), function (events) {
    for (var index = 0; index < events.length; index++) {
        var wat = events[index];
        if (wat.action == 2 /* nsfw.ActionType.MODIFIED */ && wat.file) {
            var filePath = path.resolve(wat.directory, wat.file);
